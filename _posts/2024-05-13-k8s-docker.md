...
tags:
  ...
  - Pod
  - Docker
last_modified_at: 2024-05-13T17:06:00-00:00
---
------------
...
![Dashboard](/assets/images/posts/k8s-docker/1.png)
...
참고로 `EXTERNAL-IP`가 `<pending>`에서 멈추는데, Docker Desktop의 쿠버네티스에서는 맞는 설정이라고 한다. by ChatGPT

### 쿠버네티스 포트포워딩 설정

Docker Desktop 에서는 외부 IP가 존재하지 않으므로, Ingress를 호출하기 위해서는 먼저 포트 포워딩을 설정해줘야 한다.
```powershell
